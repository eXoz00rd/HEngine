...
- The trigger for creating Foundation is content, not schedule. Likely candidates as work proceeds: metadata attributes for the component registry (`[Tooltip]`, `[Range]`, `[ComponentId]`), which §6.1 explicitly places in Foundation, and diagnostics counters (§8.5). Both are still hypothetical.
